...
if __name__ == '__main__': #the main part of the program.
    ...
    Xsmp,Chain,_,comm = run_tmcmc(Nsmp,all_params,logposterior,parallel_processing,str(data_out / 'stat-file-tmcmc_synth_case1_noise10_initial_8param.txt'))

##### IF YOU WANT TO LOAD PREVIOUSLY GENERATED SAMPLES 
    # Xsmp = np.loadtxt(data_out / 'muVec_synthetic_case2.dat')
    # Chain = np.loadtxt(data_out / 'muVec_long_synthetic_case2.dat')
    ...
    Xsmp = Xsmp.T
    np.savetxt(data_out / 'muVec_synth_case1_noise10_initial_8param.dat',Xsmp)
    np.savetxt(data_out / 'muVec_long_synth_case1_noise10_initial_8param.dat',Chain)
